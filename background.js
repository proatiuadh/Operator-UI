class BrowserController {
    ...
    truncatePrompt(prompt) {
        const maxInputTokens = this.MAX_TOKENS - this.MAX_NEW_TOKENS;
        if (prompt.length > maxInputTokens) {
            return prompt.slice(0, maxInputTokens);
        }
        return prompt;
    }
        ...
        try {
            ...
            const response = await fetch(this.API_URL, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            ...
                            - Do not repeat commands that have already been executed or given in the prompt.`
                    ...
                    max_tokens: this.MAX_NEW_TOKENS,
                    stream: false
                })
            });
    ...
    cleanAIResponse(response) {
        ...
        return response.split("\n").filter(line =>
            line.match(/^(goto:|click:|type:|submit:|scroll:|extract:|open-new-tab:|wait-for:|parse-html|get-html)/)
        );
    }
}
